...
var addContent = function(arr) {

    $.each(arr, function(i, val) {
        // console.log(val.name, val.id);

        var elem = `
        <div id="${val.alt}" class="thumb-grid item">
            <img src="/mas_assets/media/tea_collection/landing-pages/wallshoppe/2020/0506/v0/${val.name}" alt="${val.alt}">
        </div>`;
        ...
        if (theElem !== undefined) {
            if (val.onPage !== true) {
                ...
                // console.log(`${val.name} already on page`);
            }
        } else {
            // console.log(`${val.name} not on page yet`);
        }
